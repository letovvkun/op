...
const newEpisodeNotification = document.getElementById('newEpisodeNotification');
const notificationThumb = document.getElementById('notificationThumb');
const notificationEpisode = document.getElementById('notificationEpisode');
...
// --- НОВИНКА: Функции для уведомлений ---
function showNotification(episode) {
  notificationEpisode.textContent = episode.title;
  if (episode.thumb) {
    notificationThumb.innerHTML = `<img src="${episode.thumb}" alt="Обложка серии" loading="lazy">`;
  } else {
    notificationThumb.textContent = `EP ${episode.id}`;
  }
  
  newEpisodeNotification.classList.add('show');
  
  clearTimeout(notificationTimer);
  notificationTimer = setTimeout(hideNotification, 60000); // Скрывать через 1 минуту

  localStorage.setItem('lastNotifiedEpisodeId', episode.firestoreId);
}

function hideNotification() {
  clearTimeout(notificationTimer);
  newEpisodeNotification.classList.remove('show');
}

// --- НОВИНКА: Функция для проверки новизны серии ---
function isNewEpisode(dateString) {
    const parts = dateString.split('.');
    if (parts.length !== 3) return false;
    const [day, month, year] = parts;
    const episodeDate = new Date(`${year}-${month.padStart(2, '0')}-${day.padStart(2, '0')}`);
    const today = new Date();
    const diffTime = today - episodeDate;
    const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
    return diffDays <= 8; // Включаем текущий день и 7 предыдущих
}

// --- Основные функции ---
async function loadEpisodes() {
  // Скелетон-лоадеры уже в HTML, мы их не трогаем до загрузки данных
  try {
    ...
    if (episodes.length > 0) {
        ...
        const lastNotifiedId = localStorage.getItem('lastNotifiedEpisodeId');
        if (latestEpisode.firestoreId !== lastNotifiedId) {
            setTimeout(() => showNotification(latestEpisode), 1500); // Небольшая задержка
        }
    }
    ...
    if (episodes.length > 0) {
      ...
      episodesGrid.innerHTML = ''; // Очищаем скелетоны, если серий нет
...
function renderList(filter = '') {
  ...
  episodesGrid.innerHTML = ''; // Очищаем скелетон-лоадеры перед рендером
  ...
  if (episodesToRender.length === 0) {
      ...
      episodesToRender.forEach(ep => {
          ...
          // --- ИЗМЕНЕНО: Добавляем класс для новых серий ---
          if (isNewEpisode(ep.date)) {
